...
Foundation_String_InitializeWithCString(Foundation_CString cString) {
  let objectSize = sizeof(struct _Foundation_String);
  let string = (struct _Foundation_String*)malloc(objectSize);
  string->_objectBase.retainCount = 1;
...
void Foundation_String_Retain(Foundation_String string) {
  Foundation_ObjectBase_Retain(&string->_objectBase);
}

void Foundation_String_Release(Foundation_String string) {
  let shouldDeallocate = Foundation_ObjectBase_Release(&string->_objectBase);
